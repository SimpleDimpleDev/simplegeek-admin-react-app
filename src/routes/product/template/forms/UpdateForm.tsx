...
type ProductTemplateUpdateFormData = {
	id: string;
	title: string;
	data: {
		title: string | null;
...
const ProductTemplateUpdateResolver = z.object({
	id: z.string().min(1),
...
export const ProductTemplateUpdateForm: React.FC<ProductTemplateUpdateFormProps> = ({ id, onSubmit }) => {
	...
	const resolvedOnSubmit = (data: ProductTemplateUpdateFormData) => {
		onSubmit(ProductTemplateUpdateSchema.parse(data));
	};
	...
	const { control, handleSubmit, setValue, watch } = useForm<ProductTemplateUpdateFormData>({
		resolver: zodResolver(ProductTemplateUpdateResolver),
